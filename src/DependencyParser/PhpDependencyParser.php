...
class PhpDependencyParser
{
    ...
    protected function setResolvedClassNames(array $ast, array $dependencies)
    {
        foreach ($dependencies as $dep)
            $dep->fqcn = $this->getResolvedClassName($ast, $dep->name);
    }
    ...
    protected function getResolvedClassName(array $ast, string $className)
    {
        /**
         * @todo
        ...
        $items = array_filter($useStatements, function (string $fqcn) use ($className) {
            $expectedPosition = strlen($fqcn) - strlen($className);
            return strripos($fqcn, $className, 0) === $expectedPosition && $fqcn[$expectedPosition - 1] === '\\';
        });
